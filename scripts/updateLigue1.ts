...
const ESPN_LIGUE1_API = "https://site.api.espn.com/apis/site/v2/sports/soccer/fra.1/scoreboard";
const LIGUE1_DURATION_MIN = 135;
...
function stableId(startUtc: string, away: string, home: string): string {
  ...
  return `soccer-ligue1-${dateStr}-${timeStr}-${a}-at-${h}`;
}
...
async function fetchEspnPage(dateRange: string): Promise<EspnEvent[]> {
  const url = `${ESPN_LIGUE1_API}?dates=${dateRange}&limit=200`;
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; SportsGuide/1.0)" },
  });
  if (!res.ok) {
    console.error(`  Ligue 1: ESPN API returned ${res.status} for ${dateRange}`);
    return [];
  }
...
export async function fetchLigue1Events(): Promise<SoccerFetchResult> {
  console.log(`  Ligue 1: Fetching from ESPN API...`);
  try {
    const now = new Date();
    ...
    console.log(`  Ligue 1: Fetched ${allEspnEvents.length} ESPN events`);
    ...
    for (const espnEvent of allEspnEvents) {
      ...
      const endUtc = addDuration(startUtc, LIGUE1_DURATION_MIN);
    ...
    console.log(`  Ligue 1: ${events.length} events in retention window`);
    return { events, sourceUsed: "espn", count: events.length };
  } catch (err: any) {
    console.error(`  Ligue 1: Error fetching ESPN API:`, err.message);
