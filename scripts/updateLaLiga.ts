...
const ESPN_LALIGA_API = "https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/scoreboard";
const LALIGA_DURATION_MIN = 135;
...
function stableId(startUtc: string, away: string, home: string): string {
  ...
  return `soccer-laliga-${dateStr}-${timeStr}-${a}-at-${h}`;
}
...
async function fetchEspnPage(dateRange: string): Promise<EspnEvent[]> {
  const url = `${ESPN_LALIGA_API}?dates=${dateRange}&limit=200`;
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; SportsGuide/1.0)" },
  });
  if (!res.ok) {
    console.error(`  La Liga: ESPN API returned ${res.status} for ${dateRange}`);
    return [];
  }
...
export async function fetchLaLigaEvents(): Promise<SoccerFetchResult> {
  console.log(`  La Liga: Fetching from ESPN API...`);
  try {
    const now = new Date();
    ...
    console.log(`  La Liga: Fetched ${allEspnEvents.length} ESPN events`);
    ...
    for (const espnEvent of allEspnEvents) {
      ...
      const endUtc = addDuration(startUtc, LALIGA_DURATION_MIN);
    ...
    console.log(`  La Liga: ${events.length} events in retention window`);
    return { events, sourceUsed: "espn", count: events.length };
  } catch (err: any) {
    console.error(`  La Liga: Error fetching ESPN API:`, err.message);
