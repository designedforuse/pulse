...
export interface SoccerFetchResult {
  events: AppEvent[];
  sourceUsed: string;
  count: number;
}

export interface SoccerMergeResult {
  merged: AppEvent[];
  added: number;
  updated: number;
  pruned: number;
...
function parseTeams(summary: string, stripRegex: RegExp): { homeTeam: string; awayTeam: string } | null {
...
function stableId(sourceKey: string, startUtc: string, away: string, home: string): string {
  ...
  return `soccer-${sourceKey}-${dateStr}-${timeStr}-${a}-at-${h}`;
}

export async function fetchSoccerLeagueEvents(config: LeagueConfig): Promise<SoccerFetchResult> {
  const label = config.league;
  console.log(`  ${label}: Fetching iCal from ${config.icalUrl}...`);
  try {
    const res = await fetch(config.icalUrl, {
      headers: { "User-Agent": "Mozilla/5.0 (compatible; SportsGuide/1.0)" },
    });
    if (!res.ok) {
      console.error(`  ${label}: iCal fetch failed with ${res.status}`);
      return { events: [], sourceUsed: "none", count: 0 };
    }
    const icalText = await res.text();
    const vevents = parseIcalText(icalText);
    console.log(`  ${label}: Parsed ${vevents.length} iCal events`);
    ...
    for (const ve of vevents) {
      ...
      const teams = parseTeams(ve.summary, config.summaryStripRegex);
      if (!teams) {
        console.log(`  ${label}: Could not parse teams from "${ve.summary}"`);
        continue;
      }
      ...
      const id = stableId(config.leagueKey, startUtc, teams.awayTeam, teams.homeTeam);
    ...
    console.log(`  ${label}: ${events.length} events in retention window`);
    return { events, sourceUsed: "ical", count: events.length };
  } catch (err: any) {
    console.error(`  ${label}: Error fetching iCal:`, err.message);
    return { events: [], sourceUsed: "none", count: 0 };
  }
}

export function mergeSoccerLeagueEvents(
  existing: AppEvent[],
  fetched: AppEvent[],
  now: Date
): SoccerMergeResult {
