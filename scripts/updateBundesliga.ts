...
const ESPN_BUNDESLIGA_API = "https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/scoreboard";
const BUNDESLIGA_DURATION_MIN = 135;
...
function stableId(startUtc: string, away: string, home: string): string {
  ...
  return `soccer-bundesliga-${dateStr}-${timeStr}-${a}-at-${h}`;
}
...
async function fetchEspnPage(dateRange: string): Promise<EspnEvent[]> {
  const url = `${ESPN_BUNDESLIGA_API}?dates=${dateRange}&limit=200`;
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; SportsGuide/1.0)" },
  });
  if (!res.ok) {
    console.error(`  Bundesliga: ESPN API returned ${res.status} for ${dateRange}`);
    return [];
  }
...
export async function fetchBundesligaEvents(): Promise<SoccerFetchResult> {
  console.log(`  Bundesliga: Fetching from ESPN API...`);
  try {
    const now = new Date();
    ...
    console.log(`  Bundesliga: Fetched ${allEspnEvents.length} ESPN events`);
    ...
    for (const espnEvent of allEspnEvents) {
      ...
      const endUtc = addDuration(startUtc, BUNDESLIGA_DURATION_MIN);
    ...
    console.log(`  Bundesliga: ${events.length} events in retention window`);
    return { events, sourceUsed: "espn", count: events.length };
  } catch (err: any) {
    console.error(`  Bundesliga: Error fetching ESPN API:`, err.message);
