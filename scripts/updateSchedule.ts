...
import { fetchCyclingEvents, mergeCyclingEvents } from "./updateCycling";
import { fetchDiamondLeagueEvents, mergeDiamondLeagueEvents } from "./updateDiamondLeague";
...
import { fetchMlbEvents, mergeMlbEvents } from "./updateMlb";
...
async function main() {
  ...
  const existingCycling = loadExistingByPrefix("cycling-");
  const existingDiamondLeague = loadExistingByPrefix("athletics-diamond-league-");
  ...
  const existingMlb = loadExistingByPrefix("baseball-mlb-");
  ...
  console.log(`Existing cached Cycling events: ${existingCycling.length}`);
  console.log(`Existing cached Diamond League events: ${existingDiamondLeague.length}`);
  ...
  const cyclingFetchResult = fetchCyclingEvents();
  const diamondLeagueFetchResult = fetchDiamondLeagueEvents();
  ...
  const cyclingResult = mergeCyclingEvents(existingCycling, cyclingFetchResult.events, now);
  const diamondLeagueResult = mergeDiamondLeagueEvents(existingDiamondLeague, diamondLeagueFetchResult.events, now);
  ...
  const mlbResult = mergeMlbEvents(existingMlb, mlbFetchResult.events, now);
  ...
  console.log(`  Cycling source: ${cyclingFetchResult.sourceUsed} (${cyclingFetchResult.count} events)`);
  console.log(`  Cycling merge: +${cyclingResult.added} added, ~${cyclingResult.updated} updated, -${cyclingResult.pruned} pruned → ${cyclingResult.merged.length} total`);
  console.log(`  Diamond League source: ${diamondLeagueFetchResult.sourceUsed} (${diamondLeagueFetchResult.count} events)`);
  console.log(`  Diamond League merge: +${diamondLeagueResult.added} added, ~${diamondLeagueResult.updated} updated, -${diamondLeagueResult.pruned} pruned → ${diamondLeagueResult.merged.length} total`);
  ...
  console.log(`  NCAAB merge: +${ncaabResult.added} added, ~${ncaabResult.updated} updated, -${ncaabResult.pruned} pruned → ${ncaabResult.merged.length} total`);
  console.log(`  Intl Soccer source: ${intlSoccerFetchResult.sourceUsed} (${intlSoccerFetchResult.count} events)`);
  console.log(`  Intl Soccer merge: +${intlSoccerResult.added} added, ~${intlSoccerResult.updated} updated, -${intlSoccerResult.pruned} pruned → ${intlSoccerResult.merged.length} total`);
  ...
  const generatedMeta = {
    ...
    sources: {
      ...
      marathon: {
        count: marathonResult.merged.length,
        lastFetchAt: nowIso,
        sourceName: "Hardcoded Schedule",
        raceCounts: marathonFetchResult.raceCounts,
      },
      nba: {
      ...
      ncaab: {
        count: ncaabResult.merged.length,
        lastFetchAt: ncaabFetchResult.events.length > 0 ? nowIso : (prevMeta?.sources?.ncaab?.lastFetchAt || nowIso),
        sourceName: "ESPN API",
      },
      mlb: {
        count: mlbResult.merged.length,
        lastFetchAt: mlbFetchResult.events.length > 0 ? nowIso : (prevMeta?.sources?.mlb?.lastFetchAt || nowIso),
        sourceName: "ESPN API",
      },
    },
  };
  ...
  const output = {
    lastUpdated: nowIso,
    sources: ["NHL API (api-web.nhle.com)", "AHL (HockeyTech / Odds API fallback)", "ECHL (API-Hockey / HockeyTech web)", "NCAA (College Hockey News)", "Rugby (iCal feeds)", "Champions Cup (iCal feed)", "MLR (hardcoded)", "Cricket (CricAPI)", "Olympic Hockey (hardcoded)", "EPL (iCal feed)", "MLS (iCal feed)", "Serie A (iCal feed)", "La Liga (iCal feed)", "Bundesliga (iCal feed)", "Ligue 1 (iCal feed)", "NWSL (iCal feed)", "USL (ESPN API)", "Champions League (ESPN API)", "Europa League (ESPN API)", "FA Cup (ESPN API)", "Tennis (hardcoded)", "F1 (ESPN API)", "NBA (ESPN API)", "NCAAB Big West (ESPN API)"],
    ...
    ncaabCount: ncaabResult.merged.length,
    ncaabAdded: ncaabResult.added,
    ncaabUpdated: ncaabResult.updated,
    ncaabPruned: ncaabResult.pruned,
    ncaabSourceUsed: ncaabFetchResult.sourceUsed,
    intlSoccerCount: intlSoccerResult.merged.length,
    intlSoccerAdded: intlSoccerResult.added,
    intlSoccerUpdated: intlSoccerResult.updated,
    intlSoccerPruned: intlSoccerResult.pruned,
    intlSoccerSourceUsed: intlSoccerFetchResult.sourceUsed,
    mlbCount: mlbResult.merged.length,
    mlbAdded: mlbResult.added,
    mlbUpdated: mlbResult.updated,
    mlbPruned: mlbResult.pruned,
    mlbSourceUsed: mlbFetchResult.sourceUsed,
    generatedMeta,
    events: allEvents,
  };
