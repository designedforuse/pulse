...
const ESPN_FA_CUP_API = "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.fa/scoreboard";
const FA_CUP_DURATION_MIN = 135;
...
function stableId(startUtc: string, away: string, home: string): string {
  ...
  return `soccer-facup-${dateStr}-${timeStr}-${a}-at-${h}`;
}
...
async function fetchEspnPage(dateRange: string): Promise<EspnEvent[]> {
  const url = `${ESPN_FA_CUP_API}?dates=${dateRange}&limit=200`;
  const res = await fetch(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; SportsGuide/1.0)" },
  });
  if (!res.ok) {
    console.error(`  FA Cup: ESPN API returned ${res.status} for ${dateRange}`);
    return [];
  }
...
export async function fetchFaCupEvents(): Promise<SoccerFetchResult & { firstMatchDate?: string }> {
  console.log(`  FA Cup: Fetching from ESPN API...`);
  try {
    const now = new Date();
    ...
    console.log(`  FA Cup: Fetched ${allEspnEvents.length} ESPN events`);
    ...
    for (const espnEvent of allEspnEvents) {
      ...
      const endUtc = addDuration(startUtc, FA_CUP_DURATION_MIN);
      ...
      events.push({
        ...
        source: "soccer-facup",
        leagueKey: "facup",
        providerReason: "facup-disneyplus",
      });
    }

    console.log(`  FA Cup: ${events.length} events in retention window`);
    if (firstMatchUtc) {
      console.log(`  FA Cup: First match in feed: ${firstMatchUtc}`);
    }
    return { events, sourceUsed: "espn", count: events.length, firstMatchDate: firstMatchUtc ?? undefined };
  } catch (err: any) {
    console.error(`  FA Cup: Error fetching ESPN API:`, err.message);
    return { events: [], sourceUsed: "none", count: 0 };
  }
}

export function mergeFaCupEvents(existing: any[], fetched: any[], now: Date): SoccerMergeResult {
