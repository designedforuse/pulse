...
- **Watch Tab (4-Game Chaos Setup):** The primary tab answers "What should I be watching?" with a multiview module showing 1 primary featured card + up to 3 secondary cards. Candidate pool: events overlapping NOW..NOW+90min. Selection uses the ladder priority system with constraints (must include a favorite if any exist, highest ladder-priority match, sport diversity boost). Setup is stable per session — regenerates only on manual "Rebuild Chaos" button, pull-to-refresh, or when all selected events end. A priority alert banner ("Higher priority game now live") appears when a better game starts, with an "Update Setup" action (no auto-switch). Below the chaos module, integrated Live Now and Up Next sections show all live/upcoming events sorted by favorites → start time. Module logic lives in `lib/chaos-setup.ts`.
